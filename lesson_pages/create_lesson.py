...
from modules.lessons.service import create_lesson_from_inputs
...
def build_conversation_inputs(count: int) -> list[dict[str, str | int | None]]:
    ...
    for index in range(count):
        ...
        speaker_col, kana_col = st.columns([1, 3])
        with speaker_col:
        ...
        with romaji_col:
            japanese_romaji = st.text_input(
                ...
                placeholder="yoroshiku onegaishimasu.",
            )

        _, english_col = st.columns([1, 3])
        with english_col:
...
st.caption("Create a lesson with conversation, vocabulary, and grammar")
...
if submitted:
    try:
        ...
        st.success(
            (
                f"Lesson {lesson.lesson_level}-"
                f"{lesson.lesson_number} was created successfully."
            )
        )
